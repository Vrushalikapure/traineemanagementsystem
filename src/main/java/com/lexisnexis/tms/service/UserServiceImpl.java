...
public class UserServiceImpl implements UserService {
	...
	public boolean saveUser(User user) throws NoSuchAlgorithmException {
		LOGGER.debug("tms ServiceIml class saveUser() finding user data "+user.getUserName());
		boolean existsById = userRepository.existsById(user.getUserName());
		if (existsById == false) {
			String encryptPass = passwordEncrpt.encryptPass(user.getPassword());
			user.setPassword(encryptPass);
			userRepository.save(user);
			LOGGER.debug("tms ServiceIml class saveUser() completed "+user.getUserName());
			return true;
		} else {
			LOGGER.debug("tms ServiceIml throw UserNameAlreadyExistException() class"+user.getUserName());
